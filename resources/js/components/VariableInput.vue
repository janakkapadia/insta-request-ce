...
return [];
    ...
    const variables = envMatches.map(name => {
        ...
        return {
            ...
            scope: store.activeEnvironment ? `Unresolved in ${store.activeEnvironment.name}` : 'No active environment selected'
        };
    });
    ...
    pathMatches.forEach(name => {
        ...
        const activePathVariables = props.pathVariables && props.pathVariables.length > 0 
            ? props.pathVariables 
        ...
        if (variable && variable.value) {
            ...
            variables.push({
                ...
                display: `:${name}`,
                resolved: false,
                value: variable ? 'N/A (Value is empty)' : 'N/A (Disabled or not found)',
                scope: 'Path Variable'
            });
        }
    });
    ...
    <TooltipProvider>
        <Tooltip :delay-duration="150" :open="isTooltipOpen">
            <TooltipTrigger as-child>
                <div 
                    class="relative flex items-center w-full bg-background border border-input rounded-md shadow-sm transition-colors focus-within:ring-1 focus-within:ring-ring focus-within:border-ring"
                    v-bind="$attrs"
                ...
                >
                    ...
                    <div 
                        v-if="type !== 'password'"
                        ref="urlBackgroundEl"
                        class="absolute inset-0 z-0 flex items-center px-2 font-mono text-xs pointer-events-none whitespace-pre overflow-hidden select-none"
                        style="font-family: ui-monospace, SFMono-Regular, Menlo, Monaco, Consolas, monospace !important; letter-spacing: 0px !important;"
                    ...
                    <input 
                        ...
                        :type="type"
                        @scroll="syncScroll"
                        @input="emit('input', $event); syncScroll()"
                        @focus="isFocused = true; emit('focus', $event)"
                        @blur="isFocused = false; emit('blur', $event)"
                        :placeholder="placeholder"
                        class="relative z-10 w-full h-full bg-transparent font-mono text-xs caret-foreground px-2 focus:outline-none focus:ring-0 border-none"
                        :class="[inputClass, type === 'password' ? 'text-foreground' : 'text-transparent']"
                        style="font-family: ui-monospace, SFMono-Regular, Menlo, Monaco, Consolas, monospace !important; letter-spacing: 0px !important;"
                    />
                </div>
            </TooltipTrigger>
            <TooltipContent v-if="urlVariables.length > 0" side="bottom" align="start" :side-offset="4" class="pointer-events-none text-xs max-w-sm p-3 bg-popover text-popover-foreground border shadow-md rounded-lg z-[100]">
                <div class="space-y-2">
                    <div class="text-[10px] uppercase font-bold tracking-wider text-muted-foreground border-b pb-1.5 mb-1 border-border flex items-center justify-between">
                        <span>Variables in Input</span>
                        <span class="text-[8px] opacity-75 font-mono">Hover to view values</span>
                    </div>
                    <table class="w-full text-left border-collapse">
                        <thead>
                            <tr class="text-[9px] uppercase text-muted-foreground">
                                <th class="pb-1 pr-4 font-semibold">Variable</th>
                                <th class="pb-1 pr-4 font-semibold">Value</th>
                                <th class="pb-1 font-semibold">Scope</th>
                            </tr>
                        </thead>
                        <tbody>
                            <tr v-for="v in urlVariables" :key="v.name + '-' + v.type" class="border-t border-border/50 text-[10px]">
                                <td class="py-1.5 pr-4 font-mono font-medium max-w-[100px] truncate" :class="v.resolved ? (v.type === 'path' ? 'text-sky-500' : 'text-amber-500') : 'text-rose-500'">
                                    {{ v.display }}
                                </td>
                                <td class="py-1.5 pr-4 font-mono text-muted-foreground max-w-[150px] truncate" :title="v.value">
                                    {{ v.value }}
                                </td>
                                <td class="py-1.5 font-mono text-muted-foreground max-w-[100px] truncate">
                                    {{ v.scope }}
                                </td>
                            </tr>
                        </tbody>
