...
<template>
    <div v-if="messages.length > 0" class="space-y-1.5">
        ...
        >
            <component :is="getIcon(msg.level)" class="w-3.5 h-3.5 mt-0.5 shrink-0" />
            <div class="flex-1 min-w-0">
                <span>{{ msg.message }}</span>
                <span v-if="msg.path" class="ml-1.5 opacity-60 font-mono text-[10px]">
                    {{ msg.path }}
                </span>
            </div>
        </div>
    </div>
    <div v-else class="text-xs text-muted-foreground text-center py-3 opacity-60">
